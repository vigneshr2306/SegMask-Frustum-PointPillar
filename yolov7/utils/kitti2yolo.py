...
def print_usage():
    ...
    print(
        "	<dataset-directory>: directory with train and val split "
...
if __name__ == "__main__":
    ...
    if (
        not os.path.exists(sys.argv[1] + "train/images-kitti")
        or not os.path.exists(sys.argv[1] + "train/labels-kitti")
        or not os.path.exists(sys.argv[1] + "val/images-kitti")
        or not os.path.exists(sys.argv[1] + "val/labels-kitti")
    ):
    ...
    for f in os.listdir(sys.argv[1] + "train/labels-kitti/"):
        ...
        if os.path.isfile(fname):
            ...
            w, h = img.size
            labels, coords = parse_labels(
                os.path.join(sys.argv[1] + "train/labels-kitti/" + f),
                labels_split,
                w,
                h,
            )
    ...
    for f in os.listdir(sys.argv[1] + "val/labels-kitti/"):
        ...
        if os.path.isfile(fname):
            ...
            w, h = img.size
            labels, coords = parse_labels(
                os.path.join(sys.argv[1] + "val/labels-kitti/" + f), labels_split, w, h
            )
