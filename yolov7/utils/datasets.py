...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(
        ...
        try:
            ...
            for p in path if isinstance(path, list) else [path]:
                ...
                #         # f += [p.parent / x.lstrip(os.sep) for x in t]  # local to global path (pathlib)
                    ...
                    with open(p, "r") as t:
                        ...
                        print("file", parent)
                        f += [
                            Path(parent, "images", x + ".png") for x in t
                        ]  # local to global path
                        # f += [p.parent / x.lstrip(os.sep) for x in t]  # local to global path (pathlib)
            ...
            self.img_files = sorted(
                [
                    x.as_posix()
                    for x in f
                    if x.as_posix().split(".")[-1].lower() in img_formats
                ]
            )
        ...
        if self.rect:
            ...
            self.batch_shapes = (
                np.ceil(np.array(shapes) * img_size /
                        stride + pad).astype(np.int)
                * stride
            )
                ...
                if os.path.isfile(lb_file):
                    ...
                    with open(lb_file, "r") as f:
                        ...
                        if any([len(x) > 8 for x in l]):  # is segment
                            ...
                            segments = [
                                np.array(
                                    x[1:], dtype=np.float32).reshape(-1, 2)
                                for x in l
                            ]  # (cls, xy1...)
                            l = np.concatenate(
                                (classes.reshape(-1, 1),
...
def remove_background(img, labels, segments):
    ...
    for j in range(n):
        cv2.drawContours(
            im_new, [segments[j].astype(np.int32)], -
...
def sample_segments(img, labels, segments, probability=0.5):
    ...
    if probability and n:
        ...
        for j in random.sample(range(n), k=round(probability * n)):
            ...
            cv2.drawContours(
                mask, [segments[j].astype(np.int32)], -
                1, (255, 255, 255), cv2.FILLED
            )
...
if __name__ == "__main__":
    LoadImagesAndLabels(path="/home/keshav/kitti/data/training/train.txt")
