...
def get_2d_label_anno(label_path):
    annotations = {}
    annotations.update({
        'names': [],
        'bboxes': []
    ...
    annotations['names'] = np.array([x[0] for x in content])
    num_gt = len(annotations['names'])
    ...
    if len(content) != 0 and len(content[0]) == 16:  # have score
        ...
        annotations['score'] = np.zeros((annotations['bboxes'].shape[0], ))
    return annotations
...
def evaluate(config_path,
    ...
    if ref_detfile:
        ref_dets = read_det_file(ref_detfile)
    elif det_dir:
